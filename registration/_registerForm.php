...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $userName = optimizateInput($_POST["userName"]);
    ...
    $userCategory = optimizateInput($_POST["userCategory"]);
    ...
    */
    ...
    if (empty($userName)) {
        $userNameMsg_php = "[E001] User Name must be input!";
    }

    if ($userNameMsg_php == "") {
        $_userName = db_select_user_by_userName($userName);
        if (!isset($_userName)) {
            $userNameInfoMsg_php = "[I001] User Name is acceptable!"; //NOT found UserID in DB
        } else {
            $userNameMsg_php = "[E002] The User Name is already in used; please try a different User Name!"; //Found UserID in DB
        }
    }

    if ($userNameMsg_php == "") {
        if (strlen($userName) < 5 || strlen($userName) > 50) {
            $userNameMsg_php = "[E003] User Name length must be 5 - 50 characters!";
        }
    }
    ...
    */
    if ($passMsg_php == "" && !empty($pass)) {
        ...
        if (strlen($pass) < 6 || strlen($pass) > 20) {
            $passMsg_php = "[E010] Password length must be 6 - 20 characters!";
        }
...
//        }
    ...
    if (empty($repeatPass)) {
        $repeatPassMsg_php = "[E0150] Repeat Password must be input!";
    }
...
//    }
    ...
    */
    ...
    if ($isFormDataValid) {
        ...
        $user = new User($userName,$userCategory,$encrypttedPassword);
...
////                . "://$_SERVER[HTTP_HOST]" . "/CS5281Unicorn/registration/_activateService.php"
