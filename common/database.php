<?php
	...
	function db_insert_order($order){
        try {
            ...
            $stmt->bindParam(':productID', 			$_productID);
            $stmt->bindParam(':promotionID', 			$_promotionID);
	...
	function db_prepare_order_detail($selectedProduct)
	{
		try {
			$dbconnection = db_connect();
			...
			$sql1 = "select * from `product` where productID=:selectedProduct";
			$stmt1 = $dbconnection->prepare($sql1);
			$paramArray = array(':selectedProduct' => $selectedProduct);
			
			$stmt1->execute($paramArray);
            ...
            $orderDetail4Display = new OrderDetail4Display("","","","","");

            $orderDetail4Display->setProductID($selectedProduct);
            $orderDetail4Display->setProductName($result1["productName"]);
			$orderDetail4Display->setDescription($result1["productDescription"]);
			$orderDetail4Display->setImage($result1["productImage"]);
            $orderDetail4Display->setPrice($result1["currentPrice"]);

			return $orderDetail4Display;

		}catch(PDOException $e){
		    go_to_exception_page("db_prepare_order_detail() -> ".$e);
		}
	...
	function db_select_product($selectedProduct){
        try {
            $dbconnection = db_connect();
            ...
            $product->setProductID($result1["productID"]);
            $product->setUserID($result1["userID"]);
            $product->setProductName($result1["productName"]);
            $product->setProductImage($result1["productImage"]);
            $product->setProductDescription($result1["productDescription"]);
            $product->setInventoryAmount($result1["inventoryAmount"]);
            $product->setSoldAmount($result1["soldAmount"]);
            $product->setCostPrice($result1["costPrice"]);
            $product->setInitialPrice($result1["initialPrice"]);
            $product->setCurrentPrice($result1["currentPrice"]);

            return $product;

        }catch(PDOException $e){
            go_to_exception_page("db_select_product() -> ".$e);
        }
    }
function db_select_promotion($selectedProduct){
    try {
        $dbconnection = db_connect();

        //Prepared SQL statement

        $sql = "select * from `promotion` where productID = $selectedProduct";
        ...
        $promotion->setProductID($result["productID"]);
        $promotion->setLowestPrice($result["lowestPrice"]);
        $promotion->setHighestPrice($result["highestPrice"]);
        $promotion->setEffectTime($result["effectTime"]);
        $promotion->setExpireTime($result["expireTime"]);
        $promotion->setEffectAmount($result["effectAmount"]);
        $promotion->setSoldAmount($result["soldAmount"]);

        return $promotion;

    }catch(PDOException $e){
        go_to_exception_page("db_select_promotion() -> ".$e);
    }
}

function db_update_promotion($promotionID,$soldAmount){

    $dbconnection = db_connect();

    $sql = "update `promotion` set soldAmount = $soldAmount where promotionID = $promotionID";
...
function db_update_product($productID,$soldAmount){

    $dbconnection = db_connect();

    $sql = "update `product` set soldAmount = $soldAmount where productID = $productID";
...
function db_select_order_by_OrderID($placedOrderID){

    $dbconnection = db_connect();

    $sql = "select * from `order` where orderID = $placedOrderID";
